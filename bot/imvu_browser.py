...
class IMVUBrowserClient:
    ...
    async def start(self):
        ...
        # Visit YouTube briefly to collect real browser cookies for yt-dlp
        # This bypasses YouTube's "sign in to confirm you're not a bot" error
        await self._collect_youtube_cookies()

        self.username = self.credentials.get("username", "VuTune")

    async def _collect_youtube_cookies(self):
        """Visit YouTube to get real browser cookies, save for yt-dlp to use."""
        try:
            logger.info("Collecting YouTube cookies for music playback...")
            ...
            lines = ["# Netscape HTTP Cookie File\n", "# Generated by VuTune browser\n"]
            for c in cookies:
                domain = c.get("domain", "")
                if not domain.startswith("."):
                    domain = "." + domain
                http_only = "#HttpOnly_" + domain if c.get("httpOnly") else domain
                secure = "TRUE" if c.get("secure") else "FALSE"
                expires = int(c.get("expires", 0)) if c.get("expires", 0) > 0 else 0
                lines.append(f"{http_only}\tTRUE\t{c.get('path', '/')}\t{secure}\t{expires}\t{c.get('name')}\t{c.get('value')}\n")

            cookies_path = "/tmp/yt_cookies.txt"
            with open(cookies_path, "w") as f:
                f.writelines(lines)
            logger.info(f"Saved {len(cookies)} YouTube cookies to {cookies_path}")
        except Exception as e:
            logger.warning(f"Could not collect YouTube cookies: {e}")
